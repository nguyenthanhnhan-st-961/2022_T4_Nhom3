...
begin
	insert into control.log(id_config, file_name, date, status, author)
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`nhannguyen`@`localhost` PROCEDURE `get_config`(in_id int)
begin
...
begin
	update control.log 
    set status = 'er' 
    where status = 'st' and id = (select * from (select max(id) as max from control.log) as b);
end ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
-- Dump completed on 2022-10-19 12:59:31
